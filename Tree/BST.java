...
public class BST<T extends Comparable<T>> implements SearchTree<T> {
    ...
    public void clear() {
        ...
        try {
            ...
            writeLock.unlock();
            // Now we still want to clean up references in the tree to facilitate garbage
            // collection
            // (although, since the pointer to the old root should NOT be stored anywhere by
            // now, the GC should
            // be able to identify the dangling reference and recollect the whole tree).
            // Anyway, we can clear the old tree outside of the write lock, because it's not
            // reachable anymore.
        ...
        public Optional<BSTNode> remove(T targetKey, AtomicBoolean wasRemoved) {
            if (targetKey.equals(key)) {
                ...
                if (this.isLeaf()) {
                    ...
                    // Checks if the right branch is present; if it's empty, or in 50% of the cases
                    // where both branches are present...
                    // The random component is used to prevent trees from getting skewed when
                    // several removal are performed.
                    this.getLeft().map(BSTNode::max).ifPresent(prevKey -> {
                        // We replace current node's key with the previous key in the tree (so to
                        // preserve total ordering)
                        this.key = prevKey;
                    ...
                    this.getRight().map(BSTNode::min).ifPresent(nextKey -> {
                        // We replace current node's key with the next key in the tree (so to preserve
                        // total ordering)
                        this.key = nextKey;
                        // and then remove that key from the subtree where it was found.
